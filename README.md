...
> **样式优化**：EULA 弹窗现已完全融入 Obsidian 原生模态框，无多余背景或边框，自动适配暗色/亮色主题。
...
- **时间轴视图**：按作品的播出/发售日期或你的记录日期生成纵向时间线，支持年份/季度节点，卡片点击打开笔记。工具栏会出现额外控制按钮，可切换排序依据和方向。时间轴卡片现已采用透明背景、无边框设计，仅保留左侧状态色条，视觉更轻盈。
